...
def approx(data, radius):
    ...
    for i in range(0, len(data.x_list)-1, 20):
...
def toXY(data):
    ...
    x = []
    i = 0
    while i < len(data.x_list) :
        y.append(math.sin(math.radians(data.x_list[i])) * (data.y_list[i]))
        x.append(math.cos(math.radians(data.x_list[i])) * (data.y_list[i]))
        i += 1
    return x, y
